...
void EKFquaternionToRPY(float q[4], float rpy[3]) {
    // q = [q0, q1, q2, q3] -> q0: scalar, q1,q2,q3: vector
    float q0 = q[0];
    float q1 = q[1];
    float q2 = q[2];
    float q3 = q[3];
    ...
    rpy[0] = atan2f(sinr, cosr);

    // Pitch (rotation around Y-axis)
    float sinp = 2.0f * (q0*q2 - q3*q1);
    if (fabsf(sinp) >= 1.0f)
        rpy[1] = copysignf(M_PI/2.0f, sinp); // limit to ±90°
    else
        rpy[1] = asinf(sinp);
    ...
    rpy[2] = atan2f(siny, cosy);
}


void EKFquaternionToZYZ(float q[4], float euler[3]) {
    float q0 = q[0];
    float q1 = q[1];
    float q2 = q[2];
    float q3 = q[3];
    ...
    // Z-Y-Z convention
    float alpha = atan2f(q2*q3 + q0*q1, q0*q2 - q1*q3);  // first Z
    float beta  = acosf(q0*q0 + q3*q3 - q1*q1 - q2*q2);  // Y
    float gamma = atan2f(q2*q3 - q0*q1, q0*q2 + q1*q3);  // second Z
...
void EKFquaternionToTilt(float q[4], float rpy[2]) {

    float q0 = q[0];
    float q1 = q[1];
    float q2 = q[2];
    float q3 = q[3];
    float roll_eff  = 2*(q0*q1 + q2*q3);  // componente X
    float pitch_eff = 2*(q0*q2 - q1*q3);  // componente Y
    rpy[0] = 100.0*roll_eff; // z1
    if (pitch_eff < 0) rpy[1] = 220.0*pitch_eff;
    else rpy[1] = 300.0*pitch_eff;
}
